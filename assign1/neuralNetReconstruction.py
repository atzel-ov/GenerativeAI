...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        true_sdf_tensor = data['gt_sdf'].to(device)

        estimation_clamped = torch.clamp(pred_sdf_tensor, -0.1, 0.1)
        trueval_clamped = torch.clamp(true_sdf_tensor, -0.1, 0.1)

        loss = torch.mean(torch.abs(estimation_clamped - trueval_clamped))
